...
export async function notifyNewLead(lead) {
  ...
  if (!user || !pass) {
    console.warn(
      ...
      '    Set ADMIN_EMAIL too if the admin should be copied on every enquiry.\n' +
  ...
  // ── WHO GETS TOLD ───────────────────────────────────────────
  // Every enquiry goes to the enquiries inbox AND to the admin, so a
  // lead is never sitting unseen because one mailbox went unchecked.
  // ADMIN_EMAIL accepts a comma-separated list, so several people can
  // be copied. Duplicates are collapsed — nobody gets the same lead
  // twice just because CONTACT_TO and ADMIN_EMAIL happen to match.
  const parse = (v) => (v || '').split(',').map((a) => a.trim()).filter(Boolean);

  const primary = parse(process.env.CONTACT_TO)[0] || site.email;
  const admins = parse(process.env.ADMIN_EMAIL);

  const seen = new Set([primary.toLowerCase()]);
  const recipients = [primary];
  for (const a of [...parse(process.env.CONTACT_TO).slice(1), ...admins]) {
    if (!seen.has(a.toLowerCase())) { seen.add(a.toLowerCase()); recipients.push(a); }
  }
  ...
  console.log('Lead email sent to', recipients.join(', '));
